...
<head>
    ...
    <style>
        ...
        .status-circle {
            ...
            border-radius: 50%;
            margin: 0 auto;
            /* Mengatur margin secara otomatis agar lingkaran berada di tengah */
        }
    </style>
                ...
                <div class="container-fluid px-4">
                    ...
                    if (isset($_GET['region']) && array_key_exists($_GET['region'], $regions)) {
                        ...
                        foreach ($lokasiRegion as $key => $value) {
                            ...
                            echo '<td><span class="status-circle" style="background-color: ' . $statusColor . '" title="' . $value['status'] . '"></span></td>';
                            echo '<td>';

                            // TOMBOL EDIT
                            // echo '<a href="' . base_url('lokasi/editLokasi' . ucfirst($selectedRegion) . '/' . $value['no']) . '" class="btn btn-warning" style="font-size: 14px;">Edit</a>';
                            // echo '<a href="' . base_url('lokasi/editLokasi/' . $value['no'] . '/' . $selectedRegion) . '" class="btn btn-warning" style="font-size: 14px;">Edit</a>';
                            echo '<a href="' . base_url('lokasi/editLokasi/' . $selectedRegion
                                . '/' . $value['no']) . '" class="btn btn-warning" style="font-size: 14px;">Edit</a>';
                            echo '<a href="' . base_url('lokasi/deleteLokasi/' . $selectedRegion . '/' . $value['no']) . '" class="btn btn-danger" onclick="return confirm(\'Anda yakin akan menghapus data?\')" style="font-size: 14px;">Hapus</a>';
                            echo '</td>';
                            echo '</tr>';
                        }
