...
<body>
    ...
    <header id="header" class="fixed-top d-flex align-items-center" style="height: 5%; background-color: #025464;">
        <div class=" container d-flex align-items-center justify-content-between">
            ...
            <h1 class="logo" style="font-size: 14px; text-decoration: none; border-left: none; margin-right: auto; text-transform: none;"><a href="<?= base_url('Lokasi') ?>">Animasi Spatio-Temporal</a></h1>
    ...
    <div id="map">
        ...
        <script>
            var mapView = new ol.View({
                center: ol.proj.fromLonLat([110.31081117807459, -8.025607765026137]),
                zoom: 9.5,
            });
            ...
            // Menghitung tanggal mulai dan tanggal selesai dari data Anda
            var dataKeys = Object.keys(jumlahDataPerBulan);
            var tanggalMulai = new Date(dataKeys[0]);
            var tanggalSelesai = new Date(dataKeys[dataKeys.length - 1]);
            
            var chart = new CanvasJS.Chart("chartContainer", {
                title: {
                    text: "Grafik Jumlah Data per Bulan"
                },
                axisX: {
                    title: " ",
                    intervalType: "month",
                    valueFormatString: "MMM YYYY", // Menggunakan format bulan dan tahun yang lebih singkat
                    viewportMinimum: tanggalMulai, // Set tanggal mulai
                    viewportMaximum: tanggalSelesai // Set tanggal selesai
                },
                axisY: {
                    title: "Jumlah Data"
                },
                data: [{
                    type: "column", // Menggunakan jenis grafik batang
                    dataPoints: [] // DataPoints akan diisi nanti dalam fungsi updateChart()
                }]
            });
            
            // Fungsi untuk menghapus "01" dari tampilan tooltip
            function toolTipFormatter(e) {
                var content = " ";
                for (var i = 0; i < e.entries.length; i++) {
                    content += "Waktu: " + e.entries[i].dataPoint.label + "<br/>";
                    content += "Jumlah Data: " + e.entries[i].dataPoint.y + "<br/>";
                }
                return content;
            }
            ...
            // Panggil updateChart() untuk mengisi grafik dengan data awal saat halaman dimuat
            updateChart();
            ...
            function updateChart() {
                ...
                // Mengembalikan warna berdasarkan tahun
                var colorMap = {
                    2018: "#085F63",
                    2019: "#49BEB7",
                    2020: "#4FB783",
                ...
                // Menggunakan warna default (misalnya, hitam) jika tahun tidak ditemukan dalam peta warna
                return colorMap[year] || "black";
                 }
            
                // Dapatkan data yang sesuai dari objek jumlahDataPerBulan
                for (var bulanTahun in jumlahDataPerBulan) {
                    var year = new Date(bulanTahun).getFullYear(); // Dapatkan tahun dari tanggal
                    
                    // Ubah format bulan dan tahun yang hanya menampilkan bulan dan tahun (hilangkan "01")
                    var formattedDate = bulanTahun.split('-')[0] + ' ' + bulanTahun.split('-')[1];
                    dataPoints.push({
                        x: new Date(bulanTahun),
                        y: jumlahDataPerBulan[bulanTahun],
                        label: formattedDate, // Menambahkan label untuk ditampilkan saat hover
                        color: getColorByYear(year) // Atur warna data point berdasarkan tahun
                    });
                }

                // Set dataPoints ke dalam grafik
            ...
            function playOrResumeAnimation() {
                if (isAnimationStopped || isAnimationPaused) { // Periksa apakah animasi sudah dihentikan atau dalam keadaan dijeda
                    ...
                    }, 500); // Kecepatan animasi dalam milidetik (1 detik dalam contoh ini)
